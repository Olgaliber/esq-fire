...
function MY_PostsTypeWidget() {
	...
	parent::__construct(false, __('Cherry - Advanced Cycle', CHERRY_PLUGIN_DOMAIN), $widget_ops, $control_ops);
}
...
function widget($args, $instance) {
	...
	?>
		...
		<?php endif;
        		...
        		if ( $instance['date'] ) : ?>
        			<time datetime="<?php the_time('Y-m-d\TH:i'); ?>"><?php the_date('F d, Y'); ?></time>
        		<?php endif;
	  ...
	  <?php if ( $instance['more_link'] ) : ?>
		<a href="<?php the_permalink() ?>" class="btn btn-primary <?php if($instance['more_link_class']!="") {echo $instance['more_link_class'];}else{ ?>link<?php } ?>"><?php if($instance['more_link_text']==""){ _e('Read more1', CHERRY_PLUGIN_DOMAIN); }else{ ?><?php echo $more_link_text; ?><?php } ?></a>
	  <?php endif; ?>
      </div>
		</li><!--//.post-list_li -->
...
<div style="width:230px; float:left; padding-left:20px;">
  ...
  <label for="<?php echo $this->get_field_id("more_link_text"); ?>">
	...
	<input class="widefat" id="<?php echo $this->get_field_id("more_link_text"); ?>" name="<?php echo $this->get_field_name("more_link_text"); ?>" type="text" value="<?php echo esc_attr($instance["more_link_text"]); ?>" /> <span style="font-size:11px; color:#999;">(<?php _e('default: "Read more2"', CHERRY_PLUGIN_DOMAIN); ?>)</span>
  </label>
  </p>
  <p>
