...
class MY_CommentWidget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$avatar_size         = apply_filters('widget_avatar_size', empty($instance['avatar_size']) ? '70' : $instance['avatar_size']);
		...
		<ul class="comments-custom unstyled"><?php
			...
			<li class="comments-custom_li">
				<?php if(function_exists('get_avatar') && $display_avatar != 'off') {
					echo '<figure class="thumbnail featured-thumbnail"><span class="extraimg_1"><em></em>';
               	...
               	<div class="desc">
    				...
    				if($display_date != 'off') {
    					...
    					$comment_time = get_comment_time('d F, Y');
    					echo '<div class="meta_format">'.$title_date.'<time datetime="'.date('Y-m-d\TH:i:s', strtotime($comment_date.$comment_time)).'">'.$comment_time.'</time></div>';
    				}?>
	...
	function form($instance) {
		...
		<p><label for="<?php echo $this->get_field_id('avatar_size'); ?>"><?php _e('Avatar size (px)', CHERRY_PLUGIN_DOMAIN).":"; ?>
			<select id="<?php echo $this->get_field_id('avatar_size'); ?>" name="<?php echo $this->get_field_name('avatar_size'); ?>" style="width:80px;" >
				...
				<option value="100" <?php echo ($avatar_size === '100' ? ' selected="selected"' : ''); ?>><?php echo "100x100";?></option>
